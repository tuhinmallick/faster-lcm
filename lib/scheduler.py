# This file is modified from diffuers/schedulers/scheduling_lcm.py
...
class LCMScheduler(SchedulerMixin, ConfigMixin):
    ...
    def set_timesteps(
        ...
        lcm_origin_timesteps = (
            np.asarray(list(range(1, int(original_steps * strength) + 1))) * c - 1
        )
    ...
    def step(
        ...
        if self.config.prediction_type == "epsilon":
            predicted_original_sample = sample - sigma * model_output
        ...
        if self.step_index != self.num_inference_steps - 1:
            ...
            derivative = (sample - predicted_original_sample) / sigma
            dt = sigma_down - sigma
